...
#
# Layout B (dependent): own sources live in src/; sml-complex is vendored
# under lib/ and loaded first, in dependency order.

MLTON      ?= mlton
POLY       ?= poly
BIN        := bin
CPLXDIR    := lib/github.com/sjqtentacles/sml-complex
TEST_MLB   := test/sources.mlb
SRCS       := $(wildcard $(CPLXDIR)/* src/* test/*.sml) $(TEST_MLB)
...
# its own. Load the vendored sml-complex first, then the fft sources, then the
# test driver.
poly test-poly:
